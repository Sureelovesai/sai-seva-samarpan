...
function CommunityActivityDetailsContent() {
  const router = useRouter();
  const searchParams = useSearchParams();
  const tabsRowRef = useRef<HTMLDivElement>(null);

  const [activity, setActivity] = useState<SevaActivity | null>(null);
  const [communityTabActivities, setCommunityTabActivities] = useState<Pick<SevaActivity, "id" | "title">[]>([]);
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const res = await fetch("/api/community-activities", { cache: "no-store" });
        const data = (await res.json().catch(() => [])) as unknown;
        if (!cancelled && Array.isArray(data)) {
          setCommunityTabActivities(
            data
        ...
        if (!cancelled) setCommunityTabActivities([]);
      }
    })();
    return () => {
      cancelled = true;
  ...
  useEffect(() => {
    const cur = activity?.id ?? null;
    const prev = prevActivityIdRef.current;
    if (prev !== cur) {
      prevActivityIdRef.current = cur;
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      if (!id) {
        setActivity(null);
        setLoadError("Choose an activity from Find Community Activity and open View details from there.");
        setLoading(false);
        return;
      }
      setLoading(true);
      setLoadError(null);
      try {
        const res = await fetch(`/api/community-activities/${encodeURIComponent(id)}`, { cache: "no-store" });
        const data = await res.json().catch(() => ({}));
        if (!res.ok) {
          throw new Error(typeof data?.error === "string" ? data.error : "This community activity could not be loaded.");
        }
        if (!cancelled) {
          setActivity(data as SevaActivity);
          setLoadError(null);
        }
      } catch (e: unknown) {
        if (!cancelled) {
  ...
  const selectedTabId = searchParams.get("id")?.trim() || activity?.id || "";
  const tabsMaxPage =
    communityTabActivities.length <= tabsPerPage
      ? 0
      : Math.ceil(communityTabActivities.length / tabsPerPage) - 1;
  const showTabsArrows = communityTabActivities.length > tabsPerPage;
  const pageWidthPx = tabsPerPage * TAB_WIDTH_PX + (tabsPerPage - 1) * TAB_GAP_PX;
  const stripWidthPx =
    communityTabActivities.length * TAB_WIDTH_PX + (communityTabActivities.length - 1) * TAB_GAP_PX;
  const nVisible = Math.min(communityTabActivities.length, tabsPerPage);
  const viewportWidthPx = nVisible * TAB_WIDTH_PX + (nVisible - 1) * TAB_GAP_PX;
  const maxOffsetPx = Math.max(0, stripWidthPx - viewportWidthPx);
  const slideOffsetPx = Math.min(tabsPage * pageWidthPx, maxOffsetPx);

  useEffect(() => {
    if (communityTabActivities.length === 0 || !selectedTabId || communityTabActivities.length <= tabsPerPage) return;
    const idx = communityTabActivities.findIndex((a) => a.id === selectedTabId);
    if (idx < 0) return;
    const maxP = Math.ceil(communityTabActivities.length / tabsPerPage) - 1;
    const targetPage = Math.min(maxP, Math.floor(idx / tabsPerPage));
    setTabsPage(targetPage);
  }, [tabsPerPage, communityTabActivities.length, selectedTabId]);
  ...
  const handleJoinSeva = async (e: React.FormEvent) => {
    ...
    setSignUpError(null);
    setSignUpInfo(null);
    if (!activityIdToSubmit) {
      setSignUpError("This community activity is not available to sign up.");
      return;
    }
  ...
  const canJoinSeva = !!activityIdToSubmit;
  ...
  if (!activity) {
    return (
      <div className="min-h-screen bg-[radial-gradient(ellipse_80%_50%_at_50%_-20%,rgba(120,180,220,0.4),transparent),linear-gradient(180deg,rgba(200,220,240,0.5)_0%,rgba(180,200,230,0.6)_100%)]">
        <div className="mx-auto max-w-lg px-4 py-16 text-center">
          <h1 className="text-xl font-bold text-indigo-900">Community activity not available</h1>
          <p className="mt-3 text-sm text-zinc-700">{loadError ?? "We could not load this activity."}</p>
          <Link
            href="/find-community-activity"
            className="mt-8 inline-block rounded-lg bg-indigo-600 px-5 py-2.5 text-sm font-semibold text-white shadow hover:bg-indigo-700"
          >
    ...
    <div className="min-h-screen bg-[radial-gradient(ellipse_80%_50%_at_50%_-20%,rgba(120,180,220,0.4),transparent),radial-gradient(ellipse_60%_40%_at_100%_50%,rgba(160,200,240,0.25),transparent),radial-gradient(ellipse_60%_40%_at_0%_50%,rgba(140,180,220,0.25),transparent),linear-gradient(180deg,rgba(200,220,240,0.5)_0%,rgba(180,200,230,0.6)_50%,rgba(160,190,220,0.5)_100%)]">
      <div className="mx-auto max-w-6xl px-4 py-10">
        <div className="mb-6">
          <Link
            href="/find-community-activity"
            className="text-sm font-semibold text-indigo-800 underline hover:text-indigo-950"
          >
            ← Find Community Activity
          </Link>
        </div>

        {/* Activity tabs: same pattern as Seva Details (up to 5 visible, « » slide) */}
        <div className="mx-auto w-full max-w-4xl">
          {communityTabActivities.length > 1 && (
            <div ref={tabsRowRef} className="mb-6 flex items-center gap-2">
              {showTabsArrows && (
                <button
                  ...
                  disabled={tabsPage === 0}
                  className="shrink-0 rounded border border-zinc-300 bg-white px-3 py-2 text-sm font-medium text-zinc-600 hover:bg-zinc-100 disabled:pointer-events-none disabled:opacity-40"
              ...
              <div
                ...
                style={{
                  width:
                    Math.min(communityTabActivities.length, tabsPerPage) * TAB_WIDTH_PX +
                    (Math.min(communityTabActivities.length, tabsPerPage) - 1) * TAB_GAP_PX,
                }}
              >
                <div
                ...
                >
                  {communityTabActivities.map((a) => (
                    <button
                      key={a.id}
                      type="button"
                      onClick={() =>
                        router.push(`/community-activity-details?id=${encodeURIComponent(a.id)}`)
                      }
        ...
        {/* Community Activity Details — same layout as Seva Details for consistency */}
