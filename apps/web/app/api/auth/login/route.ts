...
export async function POST(req: Request) {
  try {
    ...
    // `token` is also returned in the body so native (mobile) clients can store it
    // and send it back as `Cookie: seva_session=<token>` (browsers continue using the httpOnly cookie below).
    const res = NextResponse.json({ ok: true, token, user: { id: user.id, email: user.email, firstName: user.firstName, lastName: user.lastName, name: user.name } });
